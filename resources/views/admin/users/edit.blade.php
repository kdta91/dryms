...
<div class="container">
    <div class="row">
        ...
        <form action="/admin/users/{{ $user->id }}" method="post" class="w-100">
            @csrf
            @method('PATCH')
            ...
            <div class="form-group row col-md-6">
                <label for="password">{{ __('Password') }}</label>
                    ...
                    <input id="password" type="password" class="form-control @error('password') is-invalid @enderror" name="password" value="{{ old('password') ?? $user->password }}" autocomplete="password" autofocus>
            ...
            <div class="form-group row col-md-6">
                <label for="usertype_id">{{ __('Usertype') }}</label>
                    ...
                    <select id="usertype_id" name="usertype_id" class="form-control @error('usertype_id') is-invalid @enderror" autofocus>
                        ...
                        <option value="{{ $usertype->id }} {{ ($usertype->id === $user->usertype_id) ? 'selected' : '' }}">{{ $usertype->type }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group row col-md-6 d-flex justify-content-end">
                ...
                <a href="/admin/users" class="btn btn-secondary m-1">Cancel</a>
            </div>
        </form>
    </div>
</div>
